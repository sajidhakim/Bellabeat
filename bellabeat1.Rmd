...
## Analysis Plan:
Having reviewed the datasets I am curious to see whether there are any trends visible for daily activity (improvement over the period). I'm going to explore trends for each participant in terms of Total Distance, Total Activity Time, and finally, Activity Speed (Total Distance/Total Activity Time) to see if any trends develop. The results of this analysis will guide to further steps.  
...
ggplot(data=activity_vs_sleep)  + 
  ...
  annotate("text", x=100, y = 80, 
           label = "It seems 50 minutes of intense activity gets you to sleep fast", 
           color="magenta", fontface='bold', size=3, angle=0)+
...
ggplot(data=no_toss_turn)  + 
  ...
  stat_regline_equation(label.x=500, label.y=70, aes(x=SedentaryMinutes, y=sleep_percent,
                            label = ..rr.label..))

```
  
None of the activity levels seem to have a correlation with time spent in bed awake.
...
## Insights:
1. Few participants (18%) consistently workout to meet the American Heart Association guidelines.  
2. Fitbit monitors heart rate but the data is not summarized in a way to track improvement in cardiovascular health or compare with American Heart Association guidelines.  
3. Few particpants log their weight. Further research is necessary to find out the reason. Is it inconvenient? Is there a disincentive? Other reasons?  
4. Logging of sleep is lacking. Further research is necessary to find the reasons.  
5. Activity level does not have any impact on how quickly participants fall off to sleep once they go to bed.  
  
## Recommendations:  
1. It has been documented that risks to cardiac health of women has been historically downplayed but medical professionals are now emphasizing it. Provide an easy way for customers to track how long they have maintained their target heart rate and how many more minutes they need to meet the American Heart Association's weekly guidelines for good cardiovascular health.  
2. Encourage and make logging and tracking of weight easier. An example would be a built-in camera that the customer can use to take a picture of the weight on a digital weight scale and automatically enter it into the weight log with just one click. One possible way to encourage customers to weigh themselves would be to have an option to have a reminder icon or beep go off consistently at chosen time of day. 
3. Similar to logging of weight, find ways to allow customers to track sleep more conveniently. Emphasize the relationship between sleep quality and quantity with health and beauty. 
